...
class VOLO(nn.Module):
    ...
    def __init__(
        ...
        #self.encoder= nn.TransformerEncoderLayer(768,4,768,0.5)
    ...
    def forward_head(self, x, pre_logits: bool = False,istrain: bool = False):
        ...
        if istrain:
            pre_out = out
            out = self.encoder(out.unsqueeze(1)).squeeze(1)
            out = torch.cat([pre_out, out], dim=0)
