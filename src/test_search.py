...
def main():
    ...
    for checkpoint_path in checkpoint_list:
        if checkpoint_path.split('-')[0] != "checkpoint":continue
        ...
        for nuisance in val_nuisances:
            ...
            # scores = pd.concat([scores['imgs'], preds_raw], axis=1)
        ...
        meanscore = sum(score_list)/len(score_list)
        if iid_score>0.911:
            print("Not right! : ",meanscore)
        else:
            print("That is right! : ",meanscore)
            if best_value < meanscore:
                best_value = meanscore
                best_name = args.checkpoint+checkpoint_path
        iidvalue_list.append(iid_score)
        value_list.append(meanscore)
        name_list.append(checkpoint_path)
    print(iidvalue_list)
    print(value_list)
    print(name_list)
